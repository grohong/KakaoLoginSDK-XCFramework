...
public class AuthController {
    ...
    static public func isValidRedirectUri(_ redirectUri:URL) -> Bool {
        return redirectUri.absoluteString.hasPrefix(KakaoCommon.shared.redirectUri())
    }
        ...
        if let url = url {
            ...
            if #available(iOS 12.0, *) {
                let authenticationSession = ASWebAuthenticationSession(url: url,
                                                                       callbackURLScheme: (try! KakaoCommon.shared.scheme()),
...
extension AuthController {
    ...
    public func makeParametersForTalk(prompts: [Prompt]? = nil,
        ...
        parameters["client_id"] = try! KakaoCommon.shared.appKey()
        parameters["redirect_uri"] = KakaoCommon.shared.redirectUri()
        ...
        if let approvalType = KakaoCommon.shared.approvalType().type {
...
extension AuthApi {    
    ...
    public func certToken(code: String,
                          ...
                          redirectUri: String = KakaoCommon.shared.redirectUri(),
                          completion:@escaping (CertTokenInfo?, Error?) -> Void) {
                API.responseData(.post,
                                Urls.compose(.Kauth, path:Paths.authToken),
                                parameters: ["grant_type":"authorization_code",
                                             "client_id":try! KakaoCommon.shared.appKey(),
                                             "redirect_uri":redirectUri,
                                             "code":code,
                                             "code_verifier":codeVerifier,
                                             "ios_bundle_id":Bundle.main.bundleIdentifier,
                                             "approval_type":KakaoCommon.shared.approvalType().type].filterNil(),
                                sessionType:.Auth,
