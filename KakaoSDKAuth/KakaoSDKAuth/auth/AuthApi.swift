...
final public class AuthApi {
    ...
    public static func isKakaoTalkLoginUrl(_ url:URL) -> Bool {
        if url.absoluteString.hasPrefix(KakaoCommon.shared.redirectUri()) {
            return true
        }
        return false
    }
    ...
    public func agt(completion:@escaping (String?, Error?) -> Void) {
        API.responseData(.post,
                                ...
                                parameters: ["client_id":try! KakaoCommon.shared.appKey(), "access_token":AUTH.tokenManager.getToken()?.accessToken].filterNil(),
                                sessionType:.Auth,
